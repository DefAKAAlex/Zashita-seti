...
Suricata зафиксировала подозрительную активность на сетевом уровне, а Fail2Ban проанализировал логи SSH и после 3 неудачных попыток заблокировал IP-адрес злоумышленника (10.130.0.5).

Комплексная защита работает: Suricata обнаруживает атаку, а Fail2Ban предотвращает ее продолжение, блокируя источник.
К сожалению, hydra успела подобрать пароль за счёт своей скорости и потоковости, и, будем честны, из-за простого пароля.
Что лишний раз показывает насколько важны сложные пароли!